...
#'
#' @examples
#' time_isochrone <- make_time_isochrone(site = 'London Bridge Station, London, UK', 'out', time = 30, multiplier = 0.7, mode = 'transit', api_key = api_key)
#'
#'
#' @export
make_isochrone <- function(site, time, direction = 'out', detail = 'medium', multiplier = 0.8,
  ...
  lats_init <- seq(from=-extents_guess$lat, to = extents_guess$lat, length.out = 10) + site$lat
  lngs_init <- seq(from=-extents_guess$lng, to = extents_guess$lng, length.out = 10) + site$lng
  ...
  if(method == 'google_guess'){
    chrone_map <- leaflet::leaflet(shp_init) %>%
      addPolygons(col='blue',fillColor = 'yellow') %>%
      addTiles() %>%
      addMarkers(data=site) %>%
      addCircles(data=df_init,lng = ~lng,lat=~lat,col='grey',radius = 0.2,popup=~paste0(lng,', ',lat,'  TIME:',minutes))
  ...
  shp_range <- shp_max %>% st_difference(shp_min)
